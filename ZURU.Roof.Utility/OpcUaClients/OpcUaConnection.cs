...
namespace ZURU.Roof.Utility.OpcUaClients
{
    public enum ExitCode : int
    {
        ...
        ErrorRunning = 0x18,
        FinishedCreateConnection = 0x19,
        ErrorNoKeepAlive = 0x30,
        ErrorReadValue = 0x40,
        ErrorSessionReadValue = 0x41,
        SuccessReadValue = 0x42,
        InitialReadValue = 0x43,
        FinishedReadValue = 0x44,
        ErrorWriteValue = 0x50,
        InitialWriteValue = 0x51,
        ErrorSessionWriteValue = 0x52,
        SuccessWriteValue = 0x53,
        FinishedWriteValue = 0x54,
        SuccessCreate = 0x20,
        ErrorInvalidCommandLine = 0x100
    };

    /// <summary>
    /// Class to be used as a helper for sending and recieving data over OPC UA
    /// </summary>
    public class OpcUaConnection
    {
        ...
        public bool IsConnected { get => mSession != null && mSession.Connected; }


        /// <summary>
        /// Constructor for OPCUAClient
        /// </summary>
        /// <param name="_endpointURL">EndPoint URL as string</param>
        /// <param name="_autoAccept">AutoAccept all certificates. Should only be used during development</param>
        /// <param name="_stopTimeout">Stop Timeout</param>
        public OpcUaConnection(string _endpointURL, bool _autoAccept, int _stopTimeout = Timeout.Infinite)
        {
            endpointURL = _endpointURL;
            autoAccept = _autoAccept;
            clientRunTime = _stopTimeout <= 0 ? Timeout.Infinite : _stopTimeout * 1000;
            logger = Log.ForContext("EndPointURL", endpointURL);
        }
        ...
        /// <summary>
        /// Create Connection to OPC UA Server and Process the Exception until it can connected.
        /// </summary>
        /// <returns></returns>
        public async Task TryConnect()
        {
            while (exitCode != ExitCode.SuccessCreate)
            {
                try
                {
                    await Connect();
                ...
                {
                    logger.Warning("---- Connected to {endpointURL} Failed!! Will reconnect in 20 seconds! Cause by: {errorMessage}", endpointURL, e.Message);
                    Thread.Sleep(20 * 1000);
                }
            }
            exitCode = ExitCode.FinishedCreateConnection;
        }

        /// <summary>
        /// Create Connection to the OPC UA Server
        /// </summary>
        /// <returns></returns>
        public async Task Connect()
        {

            logger.Information("1 - Creating OPC Application Config");
            exitCode = ExitCode.ErrorCreateApplication;

            ApplicationConfiguration config = CreateClientConfiguration();
            ...
            if (haveAppCertificate)
            {
                config.ApplicationUri = GetApplicationUriFromCertificateLocal(config.SecurityConfiguration.ApplicationCertificate.Certificate);
            ...
            {
                logger.Warning("WARN: missing application certificate, using unsecure connection.");
            }

            logger.Information("2 - Discover endpoints of {0}.", endpointURL);
            exitCode = ExitCode.ErrorDiscoverEndpoints;
            var selectedEndpoint = CoreClientUtils.SelectEndpoint(endpointURL, haveAppCertificate);//deleted last parameter of timeout
            logger.Information("    Selected endpoint uses: {0}",
                selectedEndpoint.SecurityPolicyUri.Substring(selectedEndpoint.SecurityPolicyUri.LastIndexOf('#') + 1));

            logger.Information("3 - Create a session with OPC UA server.");
            exitCode = ExitCode.ErrorCreateSession;
            var endpointConfiguration = EndpointConfiguration.Create(config);
            var endpoint = new ConfiguredEndpoint(null, selectedEndpoint, endpointConfiguration);
            mSession = await Session.Create(config, endpoint, false, "OPC UA Console Client", uint.MaxValue, new UserIdentity(new AnonymousIdentityToken()), null);
            exitCode = ExitCode.SuccessCreate;
            // register keep alive handler
            //mSession.KeepAliveInterval = 30 * 1000;
            mSession.KeepAlive += Client_KeepAlive;
        }
        ...
        /// <param name="defaultNotificationFunction">Bool for turning on default function which prints updates to console. Default False</param>
        ...
        public MonitoredItem AddMonitoredItem(Subscription subscription, string nodeIdString, string itemName, int samplingInterval, bool defaultNotificationFunction = false)
        {
            ...
            if (defaultNotificationFunction)
            {
        ...
        /// <summary>
        /// Read values by nodeId
        /// </summary>
        /// <param name="nodeIdStrings"></param>
        /// <returns></returns>
        public List<string> TryReadValues(List<string> nodeIdStrings)
        {
            List<string> readResult = new List<string>();
            while (exitCode != ExitCode.SuccessReadValue)
            {
                try
                {
                    readResult = ReadValues(nodeIdStrings);
                }
                catch (Exception e)
                {
                    exitCode = ExitCode.ErrorReadValue;
                    Thread.Sleep(3 * 1000);
                }
            }
            exitCode = ExitCode.FinishedReadValue;
            return readResult;
        }
        ...
        public List<string> ReadValues(List<String> nodeIdStrings)
        {
            ...
            exitCode = ExitCode.InitialReadValue;
            ...
            //Read the dataValues
            exitCode = ExitCode.ErrorSessionReadValue;
            ...
            exitCode = ExitCode.SuccessReadValue;
            return resultStrings;
        }

        /// <summary>
        /// write values to nodeId
        /// </summary>
        /// <param name="values"></param>
        /// <param name="nodeIdStrings"></param>
        public void TryWriteValues(List<String> values, List<string> nodeIdStrings)
        {
            while (exitCode != ExitCode.SuccessWriteValue)
            {
                try
                {
                ...
                {
                    exitCode = ExitCode.ErrorWriteValue;
                    Thread.Sleep(3 * 1000);
                }
            }
            exitCode = ExitCode.FinishedWriteValue;
        }
        ...
        {
            exitCode = ExitCode.InitialWriteValue;
            ...
            exitCode = ExitCode.ErrorSessionWriteValue;
            ...
            exitCode = ExitCode.SuccessWriteValue;
        }
        #endregion
        ...
        {
            //Console.WriteLine("Event Handler for changing monitered value..." + e.ToString());
            //ItemChangedNotification(monitoredItem, e);
            Console.WriteLine("Event Handler for changing values...");
            ...
            string data = "value: " + Utils.Format("{0}", notification.Value.WrappedValue.ToString()) +
        ...
        #region OPCUA Helpers
        private void Client_KeepAlive(Session sender, KeepAliveEventArgs e)
        {
            if (e.Status != null && ServiceResult.IsNotGood(e.Status))
            {
                logger.Information("{0} {1}/{2}", e.Status, sender.OutstandingRequestCount, sender.DefunctRequestCount);

                if (reconnectHandler == null)
                {
                    //mSession.Dispose(); //Seems do not need this
        ...
        private void Client_ReconnectComplete(object sender, EventArgs e)
        {
            ...
            mSession = reconnectHandler.Session;
        ...
        //public void ReadValues(string station2Index)
        //{
        //    throw new NotImplementedException();
        //}
        #endregion

        /// <summary>Creats a ApplicationConfiguration for OPC UA communication</summary>
        ...
        private static ApplicationConfiguration CreateClientConfiguration()
        {
            ...
            configuration.SecurityConfiguration.TrustedIssuerCertificates.StorePath = "%LocalApplicationData%/OPC Foundation/pki/issuer";
            configuration.SecurityConfiguration.TrustedPeerCertificates.StoreType = CertificateStoreType.Directory;
            configuration.SecurityConfiguration.TrustedPeerCertificates.StorePath = "%LocalApplicationData%/OPC Foundation/pki/trusted";


            //Rejected Certificate Stores
            //configuration.SecurityConfiguration = new SecurityConfiguration();
            ...
            configuration.SecurityConfiguration.RejectedCertificateStore.StorePath = "%LocalApplicationData%/OPC Foundation/pki/rejected";

            configuration.SecurityConfiguration.AutoAcceptUntrustedCertificates = true; //TODO REMOVE THIS DEBUG ONLY
            ...
            if (clientCertificate == null)
            {
                ...
                clientCertificate = CertificateFactory.CreateCertificate(
                    storeType: configuration.SecurityConfiguration.ApplicationCertificate.StoreType,
                    storePath: configuration.SecurityConfiguration.ApplicationCertificate.StorePath,
                    password: "cert",
                    applicationUri: configuration.ApplicationUri,
                    applicationName: configuration.ApplicationName,
                    subjectName: configuration.SecurityConfiguration.ApplicationCertificate.SubjectName,
                    domainNames: localIps,
                    keySize: keySize,
                    startTime: DateTime.UtcNow,
                    lifetimeInMonths: lifeTime,
                    hashSizeInBits: 256);
            }
            ...
            configuration.TransportQuotas.MaxByteStringLength = 4194304; //Needed, i.e. for large TypeDictionarys
            ...
            configuration.TraceConfiguration.OutputFilePath = "%LocalApplicationData%/Logs/Opc.Ua.CoreSampleClient.log.txt";
        ...
        public static string GetApplicationUriFromCertificateLocal(X509Certificate2 certificate)
        {
